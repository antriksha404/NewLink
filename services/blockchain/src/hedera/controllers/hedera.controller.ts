...
export class HederaController {
  ...
  @ApiOperation({
    ...
    description:
      'Creates a new Hedera account with a randomly generated private key.',
  })
  @ApiResponse({
    status: 201,
  ...
  async transferHbar(
    ...
    const { from, to, amount, privateKey } =
      transferRequest as typeof this.HederaTransferDto;

    if (!from || !to || !amount || !privateKey) {
      throw new BadRequestException(
        'Invalid request: from, to, amount, and privateKey are required.',
      );
    }
    ...
    try {
      this.logger.log(
        `Transferring ${amount} tinybars from ${from} to ${to}...`,
      );
      const status = await this.hederaService.transferHbar(
        from,
        to,
        amount,
        privateKey,
      ...
      throw new BadRequestException(
        'Failed to transfer Hbar. Please check your input and try again.',
      );
    }
  }
  ...
  @ApiOperation({
    ...
    description:
      'Fetches and returns all transactions for the specified Hedera account.',
  })
  @ApiParam({
    name: 'accountId',
    type: String,
  ...
  async listAllTransactions(
    ...
    try {
      this.logger.log(
        `Request to list all transactions for account ${accountId}.`,
      );
      if (!this.hederaService.isValidAccountId(accountId)) {
        throw new Error(`Invalid account ID: ${accountId}`);
      }

      const transaction = await this.hederaService.listAllTransactions(
        accountId,
        limit,
      );
  ...
  @ApiOperation({
    ...
    description:
      'Fetches the details of a specific transaction for the provided transaction ID using Hedera SDK.',
  })
  @ApiParam({
    name: 'transactionId',
    type: String,
    description:
      'The ID of the transaction (e.g., 0.0.12345@1680000000.123456789).',
  ...
  async getTransactionDetails(
    ...
    try {
      ...
      const transactionDetails = await this.hederaService.getTransactionDetails(
        transactionId,
      );
